...
public class GameScreen implements Screen {
    ...
    public void show() {
        ...
        resumeButtonEllipse = new Ellipse((Constants.WORLD_WIDTH - resumeButton.getWidth()) / 2f, camera.position.y, resumeButton.getWidth(), resumeButton.getHeight());
        menuButtonEllipse = new Ellipse((Constants.WORLD_WIDTH - menuButton.getWidth()) / 2f, camera.position.y - 75, menuButton.getWidth(), menuButton.getHeight());
    ...
    public void render(float delta) {
        ...
        for (Platform platform : platforms) {
            if (platform.getY() < camera.position.y - (30 + Constants.WORLD_HEIGHT / 2f)) {
        ...
        for (Monster monster : monsters) {
            if (monster.getY() < camera.position.y - (30 + Constants.WORLD_HEIGHT / 2f)) {
        ...
        for (Projectile proj : projectiles) {
            if (proj.getCircle().y > camera.position.y + Constants.WORLD_HEIGHT / 2f) {
        ...
        scoreFont.draw(batch, String.valueOf(score), 20, camera.position.y + (Constants.WORLD_HEIGHT / 2f - 10));
